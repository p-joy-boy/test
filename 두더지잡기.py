...
###########################################################################################################################
# 변수 and 함수 정의 / 변수 and 함수 정의 / 변수 and 함수 정의 / 변수 and 함수 정의 / 변수 and 함수 정의 / 변수 and 함수 정의 
...
def Lobby_setting():
    global game_start, last_change_time, click_block_time
    # 로비 설정
...
# 변수 and 함수 정의 / 변수 and 함수 정의 / 변수 and 함수 정의 / 변수 and 함수 정의 / 변수 and 함수 정의 / 변수 and 함수 정의 
###########################################################################################################################
# 게임 중 실행창 / 게임 중 실행창 / 게임 중 실행창 / 게임 중 실행창 / 게임 중 실행창 / 게임 중 실행창 / 게임 중 실행창 
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if game_start == True and game_over == False and event.type == pygame.MOUSEBUTTONDOWN and pygame.time.get_ticks() > click_block_time:
            total_click += 1
            if holes[mole_index].collidepoint(event.pos):   # mole_index의 위치 마우스 포인터 클릭 여부
                # 두더지 클릭 성공
                Mole_Click_Success()
            else:
                # 엉뚱한 곳 클릭으로 인한 놓침
    ...
    if game_start == True and game_over == False:
        Timeover_and_UpdateMolePosition()
